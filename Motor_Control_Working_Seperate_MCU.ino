...
#define MEDIUM_PHASE_DELAY 7100
#define HIGH_PHASE_DELAY 5500
#define LOW_PHASE_DELAY 7100
...
void loop() {
  ...
  if (pin_5_read == 0 && pin_7_read == 0) {
    setPhaseDelay(HIGH_PHASE_DELAY); // Stop motor
    Serial.println("Motor stopped");
  }
  else if (pin_5_read == 1 && pin_7_read == 0) {
    setPhaseDelay(LOW_PHASE_DELAY); // Low speed
    Serial.println("Low speed");
  }
  else if (pin_5_read == 0 && pin_7_read == 1) {
    setPhaseDelay(MEDIUM_PHASE_DELAY); // Medium speed
    Serial.println("Medium speed");
  }
  else if (pin_5_read == 1 && pin_7_read == 1) {
    setPhaseDelay(0); // High speed
    Serial.println("High speed");
  }

  delay(1000); // Small delay to avoid unnecessary processing
}
