...
int fadeStep = 0;             // Current step in the breathing cycle
const int fadeSteps = 500;    // Total steps in the breathing cycle
unsigned long lastUpdate = 0; // Tracks the last update time
const int updateInterval = 15; // Time (ms) between updates for smooth fading
...
void startFadeToColor(CRGB targetColor) {
    ...
    fadeStep = 0;
    fadeActive = true;
    fill_solid(leds, NUM_LEDS, CRGB::Black); // Ensure LEDs start from off
    FastLED.show();
}
...
void handleFade() {
    ...
    float angle = (PI * fadeStep) / fadeSteps; // Map step to a sine wave angle
    // int brightness = (sin(angle) * 122.5) + 132.5; // Map sine wave output to 0-255
    int brightness = (sin(angle) * 125) + 130; // Map sine wave output to 0-255
...
{
  /* if (breathEffect) {
    // Update fade color if AQI level changes during breathing effect
    static int lastAQI = -1;
    int currentAQI = calculateAQI(map_p25);
    if (currentAQI != lastAQI) {
        lastAQI = currentAQI;
        CRGB fadeColor = getFadeColor(currentAQI);
        startFadeToColor(fadeColor);
    }
    handleFade();
  } */
  ...
  if (readPMSdata(&Serial) && sysState == STATE_POWER_ON && flagTimerSet == false)
  {
    ...
    if (aqi <= PM25_LVL0_5_VAL){ // Clean Air PM25
      ...
      if (!breathEffect) {
        ...
        fill_solid(leds, NUM_LEDS, correctedColor(200, 0, 200));
        FastLED.show();
      }
    }
    else if (aqi > PM25_LVL4_VAL && aqi <= PM25_LVL5_VAL) {
